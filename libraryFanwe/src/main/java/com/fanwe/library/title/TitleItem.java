...
public class TitleItem extends LinearLayout
{
	...
	public TitleItem setTextTopColor(@ColorRes int id)
	{
		mTvTop.setTextColor(getResources().getColor(id));
		return this;
	}

	public TitleItem setTextTopColorInt(@ColorInt int id)
	{
		mTvTop.setTextColor(id);
		return this;
	}

	public TitleItem setTextTopSize( float size)
	{
		mTvTop.setTextSize(TypedValue.COMPLEX_UNIT_SP,size);
		return this;
	}
	...
	public TitleItem setImageLeftSize(int width,int height)
	{
		final int w=SDViewUtil.dp2px(width);
		final int h=SDViewUtil.dp2px(height);
		final LinearLayout.LayoutParams lp=(LayoutParams) new LayoutParams(w,h);
		mIvLeft.setLayoutParams(lp);
